...
--[[
function inv_SellWeapon(ply,command,args)
	local inv = ply.RE2Data["Inventory"]
	local cash = tonumber(ply:GetNWInt("Money"))
	local item = args[1]
	local slot = math.Round(args[2])
	print(item)
	print(slot)
	local upg = ply.RE2Data["Upgrades"]
	local intPowerLevel = tblUpg[strWeapon].pwrlvl
	local intAccuracyLevel = tblUpg[strWeapon].acclvl
	local intClipSizeLevel = tblUpg[strWeapon].clplvl
	local intFiringSpeedLevel = tblUpg[strWeapon].fislvl
	local intReloadSpeedLevel = tblUpg[strWeapon].reslvl
	local pwraddition = GAMEMODE.Weapons[strWeapon].UpGrades.Power[intPowerLevel].Price / 2
	local accaddition = GAMEMODE.Weapons[strWeapon].UpGrades.Accuracy[intAccuracyLevel].Price / 2
	local clpaddition = GAMEMODE.Weapons[strWeapon].UpGrades.ClipSize[intClipSizeLevel].Price / 2
	local fisaddition = GAMEMODE.Weapons[strWeapon].UpGrades.FiringSpeed[intFiringSpeedLevel].Price / 2
	local resaddition = GAMEMODE.Weapons[strWeapon].UpGrades.ReloadSpeed[intReloadSpeedLevel].Price / 2
...
concommand.Add("inv_SellWeapon",inv_SellWeapon)]]--
