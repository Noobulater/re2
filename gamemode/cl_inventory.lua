...
usermessage.Hook('RE2_UpdateSlot', CL_UpdateSlot);

function CL_UpdateWeaponStat( umsg )
	local weapon = umsg:ReadString()
	local stat = umsg:ReadString()
...
usermessage.Hook('RE2_UpdateWeaponStat', CL_UpdateWeaponStat);

function CL_UpdateChestSlot( umsg )
	local weapon = umsg:ReadString()
	local slot = umsg:ReadLong()
	local pwrlvl = umsg:ReadString()
	local acclvl = umsg:ReadString()
	local clplvl = umsg:ReadString()
	local fislvl = umsg:ReadString()
	local reslvl = umsg:ReadString()
...
usermessage.Hook('RE2_UpdateChestSlot', CL_UpdateChestSlot);
...
function InvMenu()
	...
	Invframe:SetPos(SW-128 - Invframe:GetWide(),ScrH()-128)
	--end
